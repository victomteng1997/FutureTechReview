...
```bash
npm install
npm run dev       # http://localhost:4321/FutureTechReview/
...
This repo is prepared for SEO coverage around crypto fintech and Bitcoin broker apps. Starter articles live under `src/content/reviews/` and use review/comparison schema, RSS, JSON feed, sitemap, and base-path-safe internal links.
...
Push this project to a GitHub repository named `FutureTechReview` and enable GitHub Pages from Actions. The workflow at `.github/workflows/deploy.yml` builds and publishes `dist/` automatically.
